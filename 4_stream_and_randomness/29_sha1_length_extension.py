...
def sha1_modified(message, h0, h1, h2, h3, h4, prefix_length):
    message += sha1.md_padding_with_mlen(len(message) + prefix_length)
    for i in range(0, len(message), 64):
        ...
        for j in range(80):
            ...
            a, b, c, d, e = ((sha1.left_rotate(a, 5) + f + e + k + w[j]) & 0xffffffff,
                             a, sha1.left_rotate(b, 30), c, d)
...
def main():
    ...
    for sec_len in range(0, 64):
        padding = sha1.md_padding_with_mlen(len(original) + sec_len)
        newmessage = original + padding + b';admin=true'
        hsh = sha1_modified(b';admin=true', a, b, c, d, e, len(original) + len(padding) + sec_len)
